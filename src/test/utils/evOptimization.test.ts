...
import { getOptimalEv, checkAllEvWaste, getTotalEvWaste } from '@/utils/evOptimization'
...
describe('evOptimization utils', () => {
  ...
  describe('getOptimalEv', () => {
    it('無駄のない努力値の場合はnullを返す', () => {
      ...
      const result = getOptimalEv('サンダー', 50, 'ひかえめ', mockIVs, evs, 'hp')
    ...
    it('0の努力値の場合はnullを返す', () => {
      ...
      const result = getOptimalEv('サンダー', 50, 'ひかえめ', mockIVs, evs, 'attack')
      expect(result).toBe(null)
    })
  })

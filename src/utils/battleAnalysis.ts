...
export const analyzeBattle = (
  ...
  const metagrossFlinchCount = results.filter(
    (r) => r.metagrossFlinched,
  ).length;
  const thunderParalyzedCount = results.filter(
    (r) => r.thunderParalyzed,
  ).length;
  const metagrossParalyzedCount = results.filter(
    (r) => r.metagrossParalyzed,
  ).length;
